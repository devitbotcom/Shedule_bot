...
| F12 | 🔴 Blocker | `.gitignore` | Passenger runtime creates `tmp/` (restart signal directory) and `stderr.log` in the app root (`~/Shedule_bot/`). Neither is in `.gitignore`, leaving `git status` dirty after deployment. A dirty working tree masks real changes — maintainer cannot reliably see what is modified on the server, and `git pull` output becomes misleading. **Found during Owner UAT.** Breaches quality primitive: **Portability / Installability** — deployment must leave the server in a clean, repeatable state; and **Maintainability / Analysability** — server state must be inspectable. Fix: add `tmp/` and `stderr.log` to `.gitignore`. **Fix pending (Developer).** |
