...
def test_message_contains_employee_name():
    ...
    assert "Іваненко О.В." in _format_message(ctx)
...
def test_date_displayed_as_dd_mm_yyyy():
    ...
    assert "07-04-2026" in _format_message(ctx)
...
def test_next_time_is_1700_for_labor():
    ...
    shift = make_shift("Alice", "Dept", "labor", "2026-04-07")
    next_ = make_shift("Bob", "Dept", "labor", "2026-04-08")
    ctx = make_ctx(shift, next_=next_)
    assert "17:00" in _format_message(ctx)


def test_next_time_is_0900_for_holiday():
    # Holiday/other next shift starts at 09:00
    shift = make_shift("Alice", "Dept", "labor", "2026-04-07")
    next_ = make_shift("Bob", "Dept", "holiday", "2026-04-08")
    ctx = make_ctx(shift, next_=next_)
    assert "09:00" in _format_message(ctx)
